...
public class ExternalStoragePersistServiceImpl implements PersistService {
    ...
    public void updateConfigInfo4Beta(ConfigInfo configInfo, String betaIps, String srcIp, String srcUser,
        ...
        try {
            ...
            jt.update(
                "UPDATE config_info_beta SET content=?, md5 = ?, src_ip=?,src_user=?,gmt_modified=?,app_name=?"+where.toString(), configInfo.getContent(), md5, srcIp, srcUser,
    ...
    public void updateConfigInfo4Tag(ConfigInfo configInfo, String tag, String srcIp, String srcUser, Timestamp time,
        ...
        try {
            ...
            jt.update(
                "UPDATE config_info_tag SET content=?, md5 = ?, src_ip=?,src_user=?,gmt_modified=?,app_name=?"+where.toString(), configInfo.getContent(), md5,
    ...
    public void updateMd5(String dataId, String group, String tenant, String md5, Timestamp lastTime) {
        ...
        try {
            ...
            jt.update(
                "UPDATE config_info SET md5 = ? "+where.toString(),
                paramList.toArray());
        } catch (CannotGetJdbcConnectionException e) {
            LogUtil.FATAL_LOG.error("[db-error] " + e.toString(), e);
            throw e;
        ...
        tjt.execute(status -> {
            try {
                ...
                if (configInfo != null) {
                    ...
                    jt.update("DELETE FROM config_info_beta "+where.toString(), dataId,
                        group, tenantTmp);
                }
            } catch (CannotGetJdbcConnectionException e) {
                LogUtil.FATAL_LOG.error("[db-error] " + e.toString(), e);
    ...
    public boolean addAggrConfigInfo(final String dataId, final String group, String tenant, final String datumId,
        ...
        try {
            try {
                String dbContent = jt
                    .queryForObject(select, paramList.toArray(), String.class);
    ...
    public boolean batchRemoveAggr(final String dataId, final String group, final String tenant,
        ...
        final String sql =
            "delete from config_info_aggr "+where.toString() +"and datum_id in ("
    ...
    public ConfigInfo4Beta findConfigInfo4Beta(final String dataId, final String group, final String tenant) {
        ...
        try {
            ...
            return this.jt.queryForObject(
                "SELECT ID,data_id,group_id,tenant_id,app_name,content,beta_ips FROM config_info_beta "+where.toString(),
                paramList.toArray(), CONFIG_INFO4BETA_ROW_MAPPER);
    ...
    public ConfigInfo findConfigInfo(final String dataId, final String group, final String tenant) {
        ...
        try {
            ...
            return this.jt.queryForObject(
                "SELECT id,data_id,group_id,tenant_id,app_name,content,md5,type FROM config_info " + where.toString(),
                paramList.toArray(), CONFIG_INFO_ROW_MAPPER);
        } catch (EmptyResultDataAccessException e) { // Indicates that the data does not exist, returns null.
            return null;
        } catch (CannotGetJdbcConnectionException e) {
            LogUtil.FATAL_LOG.error("[db-error] " + e.toString(), e);
...
//        String sql = "select id,data_id,group_id,tenant_id,app_name,content,type from config_info";
        ...
        try {
            return helper.fetchPage(sqlCount + where, sql + where + " and temp.rowno >=? ", paramList.toArray(), pageNo, pageSize,
                CONFIG_INFO_ROW_MAPPER);
        } catch (CannotGetJdbcConnectionException e) {
            LogUtil.FATAL_LOG.error("[db-error] " + e.toString(), e);
            throw e;
...
//        String select = "SELECT id,data_id,group_id,tenant_id,app_name,content,md5,gmt_modified,type from config_info where id > ? order by id asc limit ?,?";
    ...
    public long addConfigInfoAtomic(final long configId, final String srcIp, final String srcUser,
      ...
      /*  final String sql =
            "INSERT INTO config_info(id,data_id) VALUES(S_T_CONFIG_INFO.nextval,?)";*/
        final String sql =
        ...
        try {
            int id = jt.execute((PreparedStatementCreator) connection -> {
                PreparedStatement ps = connection.prepareStatement(sql, new String[]{"id"});
                ps.setString(1, configInfo.getDataId());
                ps.setString(2, configInfo.getGroup());
                ps.setString(3, tenantTmp);
                ps.setString(4, appNameTmp);
                ps.setBytes(5, configInfo.getContent().getBytes());
                ps.setString(6, md5Tmp);
                ps.setString(7, srcIp);
                ps.setBytes(8, StringUtils.isBlank(srcUser) ? null : srcUser.getBytes());
                ...
                ps.setBytes(15, StringUtils.isBlank(schema) ? null : schema.getBytes());
                return ps;
            }, ps -> {
                ps.executeUpdate();
    ...
    public void removeConfigInfoTag(final String dataId, final String group, final String tenant, final String tag,
        ...
        try {
            ...
            jt.update("DELETE FROM config_info_tag "+where.toString(), dataId,
    ...
    public void updateConfigInfoAtomic(final ConfigInfo configInfo, final String srcIp, final String srcUser,
        ...
        try {
           ...
           /* jt.update("UPDATE config_info SET content=?, md5 = ?, src_ip=?,src_user=?,gmt_modified=?,"
                    + "app_name=?,c_desc=?,c_use=?,effect=?,type=?,c_schema=? "
                    + where.toString(), configInfo.getContent().getBytes(), md5Tmp, srcIp, srcUser,
            ...
            jt.execute((PreparedStatementCreator) connection -> {
                ...
                ps.setBytes(1, configInfo.getContent().getBytes());
                ps.setString(2, md5Tmp);
                ps.setString(3, srcIp);
                ps.setBytes(4, StringUtils.isBlank(srcUser) ? null : srcUser.getBytes());
                ...
                ps.setBytes(11, StringUtils.isBlank(schema) ? null : schema.getBytes());
    ...
    public ConfigAdvanceInfo findConfigAdvanceInfo(final String dataId, final String group, final String tenant) {
        ...
        try {
            ...
            ConfigAdvanceInfo configAdvance = this.jt.queryForObject(
                "SELECT gmt_create,gmt_modified,src_user,src_ip,c_desc,c_use,effect,type,c_schema FROM config_info "+where.toString(),
                paramList.toArray(), CONFIG_ADVANCE_INFO_ROW_MAPPER);
        ...
        try {
            ...
            ConfigAllInfo configAdvance = this.jt.queryForObject(
                "SELECT id,data_id,group_id,tenant_id,app_name,content,md5,"
                    ...
                    + where.toString(), paramList.toArray(),
    ...
    @Override
    public void insertConfigHistoryAtomic(final long id, final ConfigInfo configInfo, final String srcIp, final String srcUser,
                                          final Timestamp time, final String ops) {
        ...
        final String md5Tmp = MD5Utils.md5Hex(configInfo.getContent(), Constants.ENCODE);
        try {
            String sql="INSERT INTO his_config_info (id,data_id,group_id,tenant_id,app_name,content,md5,src_ip,src_user,gmt_modified,op_type) "
                + "VALUES(?,?,?,?,?,?,?,?,?,?,?)";
            jt.execute((PreparedStatementCreator) connection -> {
                PreparedStatement ps = connection.prepareStatement(sql);
                ps.setLong(1,id);
                ...
                ps.setBytes(6, configInfo.getContent().getBytes());
                ps.setString(7, md5Tmp);
                ps.setString(8, srcIp);
                ps.setBytes(9, StringUtils.isBlank(srcUser) ? null : srcUser.getBytes());
                ps.setTimestamp(10, time);
                ps.setString(11, ops);
                return ps;
            }, PreparedStatement::executeUpdate);
    ...
    public Page<ConfigHistoryInfo> findConfigHistory(String dataId, String group, String tenant, int pageNo,
        ...
        String sqlFetchRows ="SELECT nid,data_id,group_id,tenant_id,app_name,src_ip,src_user,op_type,gmt_create,gmt_modified from" +
            " (select ROWNUM as rowno,t.* from (SELECT nid,data_id,group_id,tenant_id,app_name,src_ip,src_user,op_type,gmt_create,gmt_modified from his_config_info order by nid desc) t where rownum <=?) temp " +where.toString();

        Page<ConfigHistoryInfo> page;
        try {
            page = helper.fetchPage(sqlCountRows, sqlFetchRows + " and temp.rowno >=? ", paramList.toArray(), pageNo, pageSize,
    ...
    public Map<String, Object> batchInsertOrUpdate(List<ConfigAllInfo> configInfoList, String srcUser, String srcIp,
        ...
        for (int i = 0; i < configInfoList.size(); i++) {
            ...
            try {
                ParamUtils
                    .checkParam(configInfo.getDataId(), configInfo.getGroup(), "datumId", new String(configInfo.getContent()));
