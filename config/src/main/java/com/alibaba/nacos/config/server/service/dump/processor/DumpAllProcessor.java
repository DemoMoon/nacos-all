            ...
            if (page != null && page.getPageItems() != null && !page.getPageItems().isEmpty()) {
                for (ConfigInfoWrapper cf : page.getPageItems()) {
                    ...
                    if (cf.getDataId().equals(AggrWhitelist.AGGRIDS_METADATA)) {
                        AggrWhitelist.load(new String(cf.getContent()));
                    }
                    
                    if (cf.getDataId().equals(ClientIpWhiteList.CLIENT_IP_WHITELIST_METADATA)) {
                        ClientIpWhiteList.load(new String(cf.getContent()));
                    }
                    
                    if (cf.getDataId().equals(SwitchService.SWITCH_META_DATAID)) {
                        SwitchService.load(new String(cf.getContent()));
                    }
                    
                    boolean result = ConfigCacheService
                            .dump(cf.getDataId(), cf.getGroup(), cf.getTenant(), new String(cf.getContent()), cf.getLastModified(),
                                    cf.getType());
